...
def addPortal(portal):
	...
	cmd = 'XBMC.RunPlugin(' + base_url + '?mode=cache&iptvking_url=' + portal['url'] + ')';	
	li = xbmcgui.ListItem(portal['name'], iconImage='special://home/addons/pulgin.video.iptvking/fanart.jpg')
...
def vodLevel():
	...
	for i in data:
		...
		if logo != '':
			...
			logo_url = 'special://home/addons/plugin.video.iptvking/fanart.jpg';
	...
	if stop == False:
		for i in data.values():
			...
			if genre_id_main == genre_id or genre_id_main == '*':
				...
				if logo != '':
					logo_url = portal['url'] + '/iptvking_portal/misc/logos/321/' + logo;
				else:
					logo_url = 'special://home/addons/plugin.video.iptvking/fanart.jpg';
...
def playLevel():
	...
	li = xbmcgui.ListItem(title, iconImage='special://home/addons/plugin.video.iptvking/fanart.jpg', thumbnailImage=logo_url);
...
if mode is None:
	...
	iptvking_url = args.get('iptvking_url', None);
	iptvking_url = iptvking_url[0];	
	load_channels.clearCache(iptvking_url, addondir);
	...
	dp = xbmcgui.DialogProgressBG();
	dp.create('IPTV iptvking', 'Just A Second ...');
...
addon_id = 'plugin.video.iptvking'
